...
/**
 * Proxy to backend real macro data (FRED, treasury yields, inflation, monetary policy).
 * When backend is up, data is real-time from sources; otherwise frontend falls back to mock.
 */
export async function GET() {
  try {
    const res = await fetch(`${BACKEND_URL}/api/macro/macro/comprehensive`, {
      method: 'GET',
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: 'Macro service unavailable', status: res.status },
        { status: 502 }
      );
    }
    ...
    console.error('Macro comprehensive proxy error:', error);
    return NextResponse.json(
      { error: 'Macro service unavailable' },
      { status: 502 }
    );
  }
}
